...
(defn facs [x s] 
  (cons s (lazy-seq (facs (+ x 1) (* s x)))))

(take 10 (facs 2 1))

;; Lazy sequence of x to the power of n
(defn xton [x n] 
  (cons (* x n) (lazy-seq (xton x (* x n)))))
...
(defn approx-e [n] 
  (reduce + (map #(with-precision 99 (/ %1 %2)) 
                 (take n (repeat 1)) (facs 1M 1M))))
(approx-e 99M)

(defn approx-e-x [x n] 
  (+ 1M (reduce + (map #(with-precision 99 (/ %1 %2)) 
